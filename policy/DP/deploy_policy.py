...
def encode_obs(observation):
    ...
    # Add depth observations (used by rgbd configs, depth is in meters for LingBotDepth)
    head_depth = observation["observation"]["head_camera"]["depth"].astype(np.float32) / 1000.0
    obs["head_depth"] = head_depth[np.newaxis]  # (1, H, W) in meters
    if "left_camera" in observation["observation"]:
        left_depth = observation["observation"]["left_camera"]["depth"].astype(np.float32) / 1000.0
        right_depth = observation["observation"]["right_camera"]["depth"].astype(np.float32) / 1000.0
